...
**Where we are:** L2-P5 "walk-ready" prep. Plan: `C:\Users\pow-w\.claude\plans\glistening-wishing-shannon.md`. **Phases A / B / C are DONE.** A (field manual `docs/audits/L2P5_MANUAL_WALK_GUIDE.md` — Classes 1–14 + Walk Itinerary) was built a prior session; B (§CM gating) + C (known JSF/P10 violations) cleared earlier this session. The **mechanical tier is fully cleared**: code remediation batch, WMM generator repair, uppercase suffixes, and the **complete identifier-naming remediation** (~742 renames, **0 naming findings**, full house-standard compliance — incl. all QP-idiom names per the convention-conflict resolution). See CHANGELOG `2026-06-24-001` + `2026-06-25-001`, LL Entry 44.

**▶ PICK UP TOMORROW — Phase D (§RP research pass).** This is the **exit-criterion blocker** (walk-ready = A + D + corrections). Fill the concrete *judging criteria* for the four **Manual classes** — **3** (comments), **7** (class/interface design), **8** (templates), **10** (concurrency) + residues in 9/13/14 — from **PRIMARY SOURCES**: C++ Core Guidelines (C.*/CP.*), MISRA-C++/AUTOSAR titles, NASA/JPL, Holzmann, CERT, + current guidance on reviewing AI-generated code. **Run as a deep-research sitting, not from memory.** Stash raw sources in `docs/audits/L2P5_RP_SOURCES_2026-06-25.md`; fold per-class criteria back into the field manual (Classes 3/7/8/10 are currently scaffolds marked *"criteria via §RP"* — § RP section at guide line ~969). Then **Phase E (wrap)**.

**Pending corrections (this session's naming resolution → fold back; part of the exit "corrections"):**
- **Field manual Class 2** (naming) still describes the JSF over-claim as an **open finding** — it's now **resolved** (code renamed to the house standard, `.clang-tidy` config corrected to match JSF 51, QP-vs-JSF resolved: QP naming is guidance not mandate). Update to reflect remediation.
- **`RULE_VERIFIABILITY_TRIAGE.md` §459** marks the naming-over-claim *"[RESOLVED 2026-06-23: supersession recorded in CODING_STANDARDS.md]"* — but it was **never actually recorded** (verified: 0 matches). Real drift/over-claim — correct it, and note the naming is now genuinely remediated.
- **`CODING_STANDARDS.md` (PROTECTED)** — record the **"Identifier naming (QP/Samek vs JSF)" supersession** as a peer bullet in "Worked consolidation decisions" (alongside function-pointer P10-vs-JSF and nullptr-vs-JSF-175). **Needs repo-owner to name the file for editing.**

**Also note:** QP-convention-divergence tracking row is below (QP naming renamed to JSF; watch-items for if QM is ever adopted). The naming-rename driver lives in the session scratchpad (`rename_ast.py`) if any stragglers surface — but re-detect was 0.
