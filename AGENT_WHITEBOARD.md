...
- **Bug B: USB CDC wedge under sustained CLI output (2026-05-20).**
  Council 2026-05-20 (NASA/JPL + Professor + ArduPilot + Cubesat, unanimous) framed this as the LL Entry 39 sustained-output limitation re-surfacing. Repro (raw serial, post Bug A fix): after a cycle of `a/l/x/r`, second-cycle `a/l/x/r` ends with the `r` (RESET-from-ABORT) wedging the chip's USB CDC. Host gets `ClearCommError: PermissionError(13, 'The device does not recognize the command.')`; subsequent `serial.Serial(...)` opens hang indefinitely. Only a probe-reset recovers. **Cubesat panelist diagnosis:** TinyUSB CDC IRQ starved by sustained Core-0 output during the back-to-back state transitions — SDK's `stdio_usb_mutex` window stretches, host driver gives up on the next write. Same family as LL22/28/31/39 but a fresh trigger. Operationally: irrelevant in flight (no USB); high-impact on pad (operator sees rejected ARM + locked terminal, looks like a brick).
  Candidate fixes from LL Entry 39's known-limitation note: (a) rate-limit drain to ≥1ms interval between TinyUSB calls; (b) move drain to dedicated repeating-timer at bounded rate (e.g., 1kHz); (c) audit whether `stdio_usb` can be retired so we own TinyUSB mutex discipline; (d) move drain to Core 1 (where it could coordinate with multicore_lockout). Multi-session investigation — council split this as separate PR from Bug A.
