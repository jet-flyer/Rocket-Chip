...
- **Four-cycle plan — Cycle 4 IN PROGRESS (L2-P5 manual standards-walk).** L2-P5 JSF AV walk + L2-P10 CLA-RBM re-collection. Cycles 1-3 closed; gate open (R-5 + Cycle-3 closed). See CHANGELOG for cycle-by-cycle history.
  ...
  **NEXT (resume order):**
  1. **Phase B — gate-wiring (IN PROGRESS).** Done this session (committed): `build/` configured (clang-tidy runs **native Windows** with `-p build/`; only the firmware *build* needs WSL) → hook Gate 2 live; `combinator_set_evaluate` CC-30 regression fixed (`d3c8965`); **eskf.cpp/eskf_codegen.cpp `-Wshadow`/`-Wfloat-equal` gate hole fixed + positive-controlled** (`CMakeLists.txt:603-604` per-file `-O2` override was *replacing* the base flags; 8 eskf `static_assert` float== suppressed as compile-time constant-identity guards). **Remaining — the clang-tidy half:** create `scripts/audit/full_tree_clang_tidy.sh` (single source of truth; also run `scripts/audit/check_warning_gate_coverage.py` from it so the compiler-flag-coverage check rides the same gate — LL 43) → rewire `scripts/hooks/pre-commit` + protected `SESSION_CHECKLIST.md` item-17 to use it (dual-hardcode fix); add to `.clang-tidy`: `bugprone-unused-return-value.CheckedFunctions` = `i2c_bus_.*;gps_uart_.*;gps_pa1010d_.*;flash_safe_execute` (regex auto-excludes void members — scan/drain/get_debug_status; NOT rc_log) + `readability-function-size.ParameterThreshold=6`; enable confusable + reserved-id (measurement: noise-free; reserved-id needs `__StackBottom`/`__StackOneBottom` NOLINT → TP-2); **prove the script catches a planted violation** (LL 36). One bench_sim commit (hook/CMake touch trips the matrix).
