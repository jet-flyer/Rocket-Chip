...
- **Four-cycle plan status (council-approved 2026-05-15, original plan at `C:\Users\pow-w\.claude\plans\parsed-soaring-popcorn.md`):**
  ...
  - **Cycle 2 (R-5 stdio removal) — BROKEN OUT 2026-05-15** into its own dedicated plan-mode iteration. Significant rework that deserves council round 1 problem-statement-only + IRL-practice research on multi-hundred-callsite library swaps + single-dev-workflow framing (temporary-broken-state is acceptable since it's not pushed until done). Council for R-5: JPL + ArduPilot + Professor + Cubesat (user direction 2026-05-15 — swaps Student for Cubesat, whose lean-newspace lib-swap experience is more relevant). Active R-5 plan file will be in `C:\Users\pow-w\.claude\plans\` (overwrites the four-cycle plan file on entry into plan mode).
  - **Cycles 3 + 4 — STASHED** to `docs/plans/CYCLE_RESIDUALS_AFTER_R5.md`. Cycle 3 (station GPS cold-boot) runs after R-5 closes at Level-2. Cycle 4 (L2-P5 JSF AV walk + L2-P10 CLA-RBM re-collection) runs after BOTH R-5 and Cycle 3 close.
