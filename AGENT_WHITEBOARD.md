...
- **Four-cycle plan status (council-approved 2026-05-15):**
  ...
  - **Cycle 3 — RAN 2026-05-21, NO-REPRO 5/5.** Station GPS cold-boot diagnostic experiment per [`docs/plans/CYCLE_RESIDUALS_AFTER_R5.md`](docs/plans/CYCLE_RESIDUALS_AFTER_R5.md) Step 1. 5 cold-boot cycles + 1 warm baseline all succeeded; PMTK pattern was consistent (`[N,N,51] window_hit:1 init:1`). Full data + per-hypothesis attribution at [`docs/audits/CYCLE3_STATION_GPS_NO_REPRO_2026-05-21.md`](docs/audits/CYCLE3_STATION_GPS_NO_REPRO_2026-05-21.md). **Stash remains open** — the no-repro doesn't prove the user-reported failure is gone (intermittent / bench-condition-dependent are plausible). When the bug surfaces again, capture `q→b` Hardware Status BEFORE warm-rebooting (preserves the failing `g_pmtkWriteResults[]` snapshot) + bench-condition context (off-time, USB topology, ambient temp). Per LL Entry 38, no speculative fix shipped.
  - **Cycle 4 — STASHED** (L2-P5 JSF AV walk + L2-P10 CLA-RBM re-collection). Was gated on Cycle 3 closing at Level-2; with Cycle 3 NO-REPRO this session, the gate is "Cycle 3 stash closed OR user accepts running Cycle 4 against the current as-shipped state." Discussion deferred to next session.
...
- **Fault-recovery rework follow-ups (open since 2026-05-14/15 landing — commits `ed7c569` + `8baa18a`, see `docs/decisions/FAULT_RECOVERY_2026-05-14.md` for the landed design):**
  ...
  - **Audit-suite regression closure** — T2b (armed fault-inject sweep) is the remaining active gate. R-22 / R-23 / R-24 closed at Level-2; R-25-exec closes when T2b's full armed sweep observes the new phase-aware fault dispatch.
