...
- **Four-cycle plan status (council-approved 2026-05-15):**
  ...
  - **Cycle 2 (R-5 stdio removal) IN PROGRESS** — Units A + B closed 2026-05-16 (10 commits). Active plan: [`docs/plans/R5_STDIO_REMOVAL.md`](docs/plans/R5_STDIO_REMOVAL.md). Format-spec inventory + ETL 20.47.1 vendored + rc_log infrastructure (hand-rolled formatter dispatching to `etl::to_string`, libc-matching round-half-to-even for floats) + DBG_PRINT macro repoint + config.h `<stdio.h>` retirement. Host ctest 800 → 827 PASS (27 rc_log tests). Vehicle banner reads byte-identical to pre-R5 baseline; binary +6072 bytes (+2.7%). **Next: Unit C (Tier 1 proving ground)** — migrate `src/safety/pyro_edge_logger.cpp` (~3 callsites) + `src/safety/fault_protection.cpp` (~2 callsites). Then Units D-J per tier order in the plan. Council round 3 amendment for float formatter (Pathfinder-class divergence avoided): hand-rolled NOT ETL.
