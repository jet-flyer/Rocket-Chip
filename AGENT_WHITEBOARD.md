...
### D3 main.cpp Refactoring — Planned, Unblocked

`main()` ~992 lines, `core1_entry()` ~367 lines. Council-approved plan in `~/.claude/plans/shimmying-snuggling-wilkes.md` (tick-function dispatcher, watchdog tracking). `.clang-tidy` created; LLVM install pending (needs admin `choco install llvm`).
...
- **clang-tidy Integration:** `.clang-tidy` config exists. Full enforcement deferred to next audit cycle. See D3 flag above for LLVM install.
