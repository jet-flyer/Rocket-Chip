...
**Session-checklist authorization skill — TODO (surfaced 2026-06-28, Claude).** The protected-file hook (now working, see CHANGELOG `2026-06-28-002`) treats **checklist-cadence** docs (`CHANGELOG.md`, `docs/PROJECT_STATUS.md`) with interim behavior: CHANGELOG additions pass silently / destructive→ask; PROJECT_STATUS any-edit→ask. The proper design is a **session-checklist skill** that, when the flow legitimately reaches the commit/session-end phase, grants add-only edits to the checklist-named cadence files (and only then) — so routine end-of-session doc updates don't prompt, while premature/out-of-cycle edits still do. This directly addresses the "Grok jumps the gun on the CHANGELOG before work is done" problem. Skill not yet built; hook's interim cadence rules hold until it is. Nathan has flagged he's wanted to build this skill anyway.
