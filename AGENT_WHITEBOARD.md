...
- **OpenRocket usage guide + broader-integration evaluation.** OpenRocket-23.09 + JDK 17 + `orhelper`/`orlab` Python bindings installed locally during IVP-131 (`C:\Users\pow-w\OpenRocket\`, `C:\Users\pow-w\Java\jdk-17.0.19+10\`). Workflow used for IVP-131: OpenRocket GUI → export trajectory CSV → `scripts/trajectory_to_sensors.py` → sensor CSVs. Write a `docs/OPENROCKET_USAGE.md` AFTER more workflow experience, covering (a) install paths + JAR location for the bench; (b) the GUI export workflow we settled on (motor mount → flight configuration → simulation → plot/export); (c) the trajectory-to-sensors converter pipeline. Then evaluate where else OpenRocket data can be piped into the toolchain — candidate uses: (i) flight-day predictions for the actual launch motor (smaller than F15-6 for <200m apogee — Estes E12/E16 class); (ii) Mission Profile parameter tuning (apogee detection thresholds derived from OpenRocket per-airframe simulations rather than hand-tuned); (iii) wind-sensitivity sweeps for abort-threshold validation; (iv) airframe stability margin checks for IVP-144 closure; (v) Monte Carlo abort-condition coverage (orlab + JPype scripting once we have a reason to batch). Surfaced 2026-05-21 user direction: "we also need to do a broader open rocket usage guide for the repo and after this evaluate where else this data can be piped in to improve out toolchain."

- **One-off code / script cleanup sweep.** Past sessions added throwaway scripts + ad-hoc test code that should have been deleted after their one-off use. Some explicit cleanup happened during testing phases, but the search wording may have missed instances (one-off code doesn't always self-identify with words like "throwaway" / "temporary"). Future session: cast a wider net — grep for stage-bounded flags (`ROCKETCHIP_STAGE_*`), `// TODO: remove`, `// XXX`, `_archive` / `_old` / `_unused` filename patterns, scripts not referenced from CMakeLists or pre-commit hook or session-checklist, and `scripts/audit/` artifacts that only made sense during the originating audit. Companion to the "Systematic dead-code / orphaned-TU detector" row below — the detector catches dead C++ TUs; this row catches dead Python scripts + dead-but-still-runnable harnesses + dead stage-flag code. Surfaced 2026-05-21 user observation: "have we checked for and cleaned up any one-off code or scripts? we did some in testing but the search might benefit from better wording."
