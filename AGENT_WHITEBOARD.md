...
- **R-25-exec audit-suite regression status (2026-05-14 bench session, updated 2026-05-15):** T0+T1+T2a passed at Level-2 in the 2026-05-14 session; T2b + T3a were deferred to the fault-recovery architecture work, which has now landed (commits `ed7c569` + `8baa18a`). Updated expected outcomes for T2b/T3a are in this row's "T2b and T3a unblocked" sub-section below; the gates are re-runnable on a future bench session.
  ...
  **T2b and T3a unblocked 2026-05-15** by the fault-recovery rework (commits `ed7c569` + `8baa18a`). Both are now re-runnable with updated expected outcomes:
  ...
  - **T3a — `warm_reboot_audit` G-W1 + G-W4** (AIRCR-only-Core-0 reset). Updated expected outcomes: in kIdle (which is where `warm_reboot_audit` runs the gate by design — fresh boot), AIRCR continues to reset cleanly via the new `fault_reset_with_visible_signal()` path. The R-19 SIO_FIFO_IRQ wedge concern doesn't get exercised because firmware no longer issues AIRCR in flight. G-W1 verifies the pad-side path; the in-flight-AIRCR path becomes implicitly non-existent (which is the architecture's intent).
...
- **In-flight fault recovery architecture rework — LANDED 2026-05-14/15** (commits `ed7c569` + `8baa18a`). The "design question" version of this row (the 5 questions surfaced 2026-05-12, the surfaced SIO_FIFO_IRQ wedge limitation, the multi-MCU framing) has been substantially executed by the rework. Plan + transcript: `C:\Users\pow-w\.claude\plans\parsed-soaring-popcorn.md` (council rounds 1+2+3 unanimous). What landed:
  ...
  **Audit-suite regression** (T2b armed fault sweep + T3a warm-reboot AIRCR gates) is the closure path for the rework's R-22 / R-23 / R-24 / R-25-exec PROBLEM_REPORTS rows. Re-runnable now that (a)+(b) are in tree with expected-outcomes updated.
