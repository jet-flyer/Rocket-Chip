...
- **Four-cycle plan — Cycle 4 IN PROGRESS (L2-P5 manual standards-walk).** L2-P5 JSF AV walk + L2-P10 CLA-RBM re-collection. Cycles 1-3 closed; gate open (R-5 + Cycle-3 closed). See CHANGELOG for cycle-by-cycle history.
  ...
  **NEXT (resume order):**
  1. **Phase B — clang-tidy gate-wiring** (the `-Werror` *compiler* gate is already done; this is the clang-tidy half): create `scripts/audit/full_tree_clang_tidy.sh` as the single source of truth for gated checks → rewire `scripts/hooks/pre-commit` + the protected `SESSION_CHECKLIST.md` item-17 to use it (fixes the dual-hardcode defect); add `.clang-tidy` `bugprone-unused-return-value.CheckedFunctions`[`i2c_bus_*`/`gps_*`/`flash_safe_execute` — NOT `rc_log` (void)] + `readability-function-size.ParameterThreshold=6`; add `__StackBottom` NOLINT; decide confusable/reserved-id enable (measurement: both noise-free now); **prove the script catches a planted violation** before trusting it (LL 36). Runs clang-tidy via WSL (Windows-native blocked — see memory `wsl-on-demand-builds`). **Watch:** the 2026-06-24 `continue`-inversions added a nesting level in the nested-scope cases (`eskf.cpp` ZUPT/GPS loops, `guard_combinator.cpp`) — when clang-tidy runs, verify `readability-function-cognitive-complexity` stays ≤ 25 (JSF-3) and decompose any function that now exceeds. Gate 2 was *skipped* at the 2026-06-24 commit (no `build/compile_commands.json`), so this is currently unchecked.
