...
## Multi-agent commit collisions — `graph.json` + cross-agent file sweep-in (OPEN, needs decision) (2026-06-29, Claude/Opus)

Recurring friction this week: separate agents' work clashing or getting swept into each other's commits. Concrete instances — Grok accidentally edited Claude's CHANGELOG entry instead of its own; an AGENTS.md edit landed in an unexpected commit; `graphify-out/graph.json` churns on every commit (the post-commit hook auto-rebuilds it) so it's easy to sweep one agent's graph state into another's unrelated commit. With Claude + Grok + Gemini all committing to shared `main`, this will keep happening.

**Lever worth evaluating — graphify's union merge-driver (NOT yet installed):** graphify ships a `merge-driver` subcommand (`graphify merge-driver %O %A %B`) that, when git hits a `graph.json` conflict, does `networkx.compose(current, other)` = the **union** of both graphs and writes it back — so parallel `graph.json` edits auto-resolve instead of producing conflict markers in an 80k-line JSON. **Important caveats found 2026-06-29 (Claude):** (1) `graphify hook install` does **NOT** auto-wire it — verified in the installed `hooks.py` `install()`, which only writes the post-commit/post-checkout hooks; wiring the merge-driver needs a manual `.gitattributes` (`graphify-out/graph.json merge=graphify`) + `git config merge.graphify.driver`. (2) The merge-driver addresses **cross-branch/parallel-commit conflicts**, which is a *different* problem from the per-commit **churn** (dirty tree after every commit) — root-caused to graphify's nondeterministic `community` field (1 node's cluster-id flips per rebuild even with `PYTHONHASHSEED=0`); graphify does not promise determinism. (3) Earlier in-session I cited graphify README quotes about the merge-driver that a WebFetch summarizer appears to have **fabricated** — the raw README does not contain them; do not trust those. **Nathan recalls the repo framing the merge-driver as the fix for this — locate the real graphify guidance (rendered GitHub page / docs/ / release notes) and quote it verbatim before acting.**

**Open decisions (deferred to a focused session):** (a) install the union merge-driver for multi-agent safety? (b) handle the `community`-field churn — gitignore the derived volatile outputs (`graph.html`, `GRAPH_REPORT.md`), canonicalize `community` in our `scripts/graphify_curate.py` (we own that script), or just `git checkout -- graphify-out/` at session end? (c) broader: a lightweight convention so agents don't sweep each other's in-flight files into unrelated commits (stage explicitly, never `git add -A` across another agent's dir). Filter/verifier/hook-wiring context: CHANGELOG `2026-06-28-003` + `docs/tools/GRAPHIFY_USAGE.md` §11.
