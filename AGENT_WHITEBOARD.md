...
- **`-Wshadow`/`-Wfloat-equal` gate silently excludes `eskf.cpp` + `eskf_codegen.cpp` (2026-06-24, Claude; caught by repo-owner positive-control instinct).** `CMakeLists.txt:598` sets `-Wpedantic;-Wshadow;-Wfloat-equal` on all `${ROCKETCHIP_SOURCES}`, but `:603-604` re-set eskf.cpp/eskf_codegen.cpp COMPILE_OPTIONS to `"-O2;-Wpedantic"` — and `set_source_files_properties` **replaces**, not appends → the two flight-critical fusion files lose shadow/float-equal. Confirmed in BOTH the WSL and Windows `build/` compile commands (CMake source is the cause, not stale config). So CHANGELOG `2026-06-24-001`'s "0 warnings" did **not** cover these two files (true only for files that kept the flags). **Fix:** `:603-604` → `"-O2;-Wpedantic;-Wshadow;-Wfloat-equal"`, rebuild, remediate any surfaced shadow/float== in eskf.cpp (eskf_codegen.cpp is generated → fix the generator if it surfaces, per CG-1), then **positive-control it** (plant a shadow + a float== in each file → build must fail `-Werror=shadow`/`-Werror=float-equal` → revert). **Lesson:** any per-file `set_source_files_properties(... COMPILE_OPTIONS ...)` override silently drops the base flags — audit every such override for dropped warnings. (This is a second gate-infra "looks-covered-but-isn't" finding alongside the dual-hardcode defect.)
...
- **Four-cycle plan — Cycle 4 IN PROGRESS (L2-P5 manual standards-walk).** L2-P5 JSF AV walk + L2-P10 CLA-RBM re-collection. Cycles 1-3 closed; gate open (R-5 + Cycle-3 closed). See CHANGELOG for cycle-by-cycle history.
  ...
  **NEXT (resume order):**
  1. **Phase B — gate-wiring (IN PROGRESS).** Done this session: `build/` configured (clang-tidy runs **native Windows** with `-p build/`; only the firmware *build* needs WSL) → hook Gate 2 now live; `combinator_set_evaluate` CC-30 regression fixed + committed (`d3c8965`); positive-control proof of the `-Werror` compiler gate run — it **surfaced the eskf shadow/float-equal hole** (see High-priority finding above). **Resume order: (0) FIX the eskf gate hole FIRST** (`CMakeLists.txt:603-604` + rebuild + remediate + positive-control). Then the clang-tidy half: create `scripts/audit/full_tree_clang_tidy.sh` (single source of truth) → rewire `scripts/hooks/pre-commit` + protected `SESSION_CHECKLIST.md` item-17 to use it (dual-hardcode fix); add to `.clang-tidy`: `bugprone-unused-return-value.CheckedFunctions` = `i2c_bus_.*;gps_uart_.*;gps_pa1010d_.*;flash_safe_execute` (regex auto-excludes void members — scan/drain/get_debug_status; NOT rc_log) + `readability-function-size.ParameterThreshold=6`; enable confusable + reserved-id (measurement: noise-free; reserved-id needs `__StackBottom`/`__StackOneBottom` NOLINT → TP-2); **prove the script catches a planted violation** (LL 36). One bench_sim commit (hook/CMake touch trips the matrix).
