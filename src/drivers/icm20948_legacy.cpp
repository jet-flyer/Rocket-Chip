...
static bool select_bank(icm20948_t* dev, uint8_t bank) {
    // SparkFun ICM_20948 / ArduPilot Invensensev2: skip the 0x7F write
    // when already on this bank. Invalidate on a failed write.
    if (dev->current_bank == bank) {
        return true;
    }
    if (i2c_bus_write_reg(dev->addr, kRegBankSel, (bank << 4)) != 0) {
