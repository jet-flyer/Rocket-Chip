...
static PyroEdgeEvent g_buffer[kPyroEdgeBufferSize];
...
static void gpio_edge_callback(uint gpio, uint32_t events) {
    ...
    uint32_t idx = g_count;
    g_buffer[idx].timestamp_us = time_us_64();
    g_buffer[idx].gpio_pin = static_cast<uint8_t>(gpio);
    g_buffer[idx].rising = (events & GPIO_IRQ_EDGE_RISE) != 0;
    g_count = idx + 1;
}
...
void pyro_edge_logger_dump_cli() {
    ...
    for (uint32_t i = 0; i < n; ++i) {
        const auto& e = g_buffer[i];
        const char* pin_name = (e.gpio_pin == g_droguPin) ? "DROGUE" :
                               (e.gpio_pin == g_mainPin)  ? "MAIN"   : "?";
