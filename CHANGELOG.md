...
**IVP-131 host-side verification: pivot from per-row ESKF state-tolerance to phase-window FD test.** Original plan implemented partway, hit ESKF innovation-gate divergence on synthesized OR trajectories (state drifts → 3σ gate trips → open-loop integration → ~50km Z error by trajectory end). Cross-reference research (curl-driven against ArduPilot Tools/Replay docs + source, PX4 ECL #14651, JPL MONTE) confirmed structural mismatch: per-row state-tolerance regression is reserved for real-flight log replay; synthesized trajectories use phase-window event assertions. Two councils ran this session — initial 4-persona on the doomed approach (approved-with-amendments), pivot 4-persona on the replacement (approved-with-8-amendments, all folded in). Plan provenance + full council transcripts at [`C:\Users\pow-w\.claude\plans\lexical-zooming-hejlsberg.md`](C:\Users\pow-w\.claude\plans\lexical-zooming-hejlsberg.md). Single commit `3bdd71f`.

Replacement: [`test/test_flight_director_replay.cpp`](test/test_flight_director_replay.cpp) (~570 LOC) drives the FD state machine through an OR-derived `FusedState` stream, dispatches operator signals at hardcoded times, and lets `flight_director_evaluate_guards()` auto-fire guard signals. 6 Big Daddy fault profiles (F15-6 nominal, early burnout, baro stuck-at-sea-level, GPS dropout in descent, IMU zero fault, baro slow drift) + 1 differential test (cleared health bit must cause divergence, not frozen velocity). Dual invariants asserted per profile: (a) phase ordering with no out-of-order transitions, (b) each transition timestamp within its window. Apogee + burnout windows derived from OR CSV at test setup; operator-event windows hardcoded ±100 ms. Sensor-lying profile added per NASA/JPL council FMEA gap (sensor-channel-dead-only is incomplete coverage). Test-file disclaimer header explicitly declares what is NOT verified: ESKF math (covered by `test_eskf_*.cpp` + `test_replay_regression.cpp`), pyro hardware (covered by `bench_sim.py`), FD response to real sensor noise (no host-side proxy). 879/879 host ctest PASS (5 existing synthetic replay tests unaffected + 7 new IVP-131 entries). Host-only work, zero touches to `src/` or `include/`, no firmware-affecting paths.

Toolchain additions: [`scripts/trajectory_to_sensors.py`](scripts/trajectory_to_sensors.py) (OR trajectory → sensor CSV converter, kept as future utility for the ESKF-replay path that will exist once real flight logs do). OpenRocket 23.09 + Adoptium JDK 17 + orlab 0.2.7 installed locally at `C:\Users\pow-w\OpenRocket\` + `C:\Users\pow-w\Java\jdk-17.0.19+10\` — verified bootable end-to-end against the existing `estes_big_daddy.rkt`. Research-driven version choice: orlab issue #2 documents 24.12 namespace breakage, so 23.09 is the modern-enough-to-trust target until orlab catches up. OR export workflow exercised end-to-end: GUI motor-mount setup, F15-6 flight configuration with +40g nosecone override for avionics payload, plot/export to CSV. Trajectory-to-sensors converter cross-checked against OpenRocket community sources (verified at OR source `RK4SimulationStepper.java` + `SimulationStatus.java`: vertical accel is NED-up positive, vertical orientation is elevation from horizontal not zenith angle, air pressure default unit is mbar, launch-rod constraint reports unconstrained accel pre-liftoff). Files: [`tests/replay_profiles/openrocket_export/big_daddy_f15_6_nominal.csv`](tests/replay_profiles/openrocket_export/big_daddy_f15_6_nominal.csv), [`tests/replay_profiles/RC_estes_big_daddy.rkt`](tests/replay_profiles/RC_estes_big_daddy.rkt), [`tests/replay_profiles/CONTENT_VALIDATION.md`](tests/replay_profiles/CONTENT_VALIDATION.md).

Backed out (council amendment #6 + LL Entry 36 — unused test data rots): the 5 pre-existing `big_daddy_*.csv` files with bogus accelerometer physics. The OR export + converter regenerates them on demand if a future session needs the sensor CSVs as input. The pivot reads the OR export directly, not the synthesized sensor CSVs.

R-28 advanced from `analyzed → DEFER` to `verified (Level-1) → REMEDIATE-DONE` in [`docs/PROBLEM_REPORTS.md`](docs/PROBLEM_REPORTS.md) with three explicit reopen triggers: (a) IVP-135a lands and real-flight log replay becomes feasible — the ArduPilot Tools/Replay pattern is the correct shape once real logs exist; (b) new ESKF state machine kind added that doesn't have a guard path the current profiles exercise; (c) supersonic-capable airframe added (transonic regime needs sidecar-exclusion infrastructure deferred from this PR). Real-flight log capture infrastructure stays at IVP-135a per existing Stage 17 plan — pulling it forward would prematurely commit schema choices that IVP-135a should own; zero overlap with pivot files (verified by file-list cross-check against `docs/plans/STAGE17_TAPERED_BUILDUP.md` lines 73-112).

Memory additions captured during the session: [feedback-no-shipping-broken-code](.claude/projects/c--Users-pow-w-Documents-Rocket-Chip/memory/feedback_no_shipping_broken_code.md), [feedback-stop-asking-about-pausing](.claude/projects/c--Users-pow-w-Documents-Rocket-Chip/memory/feedback_stop_asking_about_pausing.md), [feedback-dont-jump-ahead-of-user-blocker](.claude/projects/c--Users-pow-w-Documents-Rocket-Chip/memory/feedback_dont_jump_ahead_of_user_blocker.md), [feedback-no-scope-caps-in-plans](.claude/projects/c--Users-pow-w-Documents-Rocket-Chip/memory/feedback_no_scope_caps_in_plans.md).

WB updated: [`AGENT_WHITEBOARD.md`](AGENT_WHITEBOARD.md) gains the OpenRocket usage guide + broader-integration row (deferred to a future session that has more workflow experience to inform the guide).
