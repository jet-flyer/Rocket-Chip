...
**Protected files review & revert:** Reviewed recent edits to `.claude/SESSION_CHECKLIST.md`, `.claude/DEBUG_PROBE_NOTES.md`, `docs/MULTICORE_RULES.md`, and `standards/STANDARDS_AUDIT*.md` per owner request. Removed unauthorized Stage O policy block (item 19) from SESSION_CHECKLIST, deleted incremental `STANDARDS_AUDIT_2026-04-28.md`, reverted date + overly-specific MPU section from MULTICORE_RULES. Preserved general MPU stack guard / fault handler rules (single source in `fault_protection.cpp`, dual-core init, GDB naming note) in MULTICORE_RULES.md under Memory section. Kept non-redundant pre-commit/OpenOCD guidance in DEBUG_PROBE_NOTES.

**Non-ASCII check:** Scanned source for characters outside ASCII range. Primarily stylistic em-dashes (`—`) in comments; no functional impact. Degree symbol intentionally encoded safely in one printf path.

Files: `.claude/DEBUG_PROBE_NOTES.md`, `.claude/SESSION_CHECKLIST.md`, `.gitattributes`, `.gitignore`, `CHANGELOG.md`, `CMakeLists.txt`, `docs/baselines/stage_o_hw_verification_2026-04-28.md`, `docs/benchmarks/UD_BENCHMARK_RESULTS.md`, `docs/FAULT_INJECTION.md`, `docs/MULTICORE_RULES.md`, `docs/PROJECT_STATUS.md`, `include/rocketchip/shared_state.h`, `scripts/opt_ivp01_row10_dualcore_watch.gdb`, `scripts/start_openocd_pico_sdk.ps1`, `src/calibration/cal_hooks.cpp`, `src/cli/rc_os_commands.cpp`, `src/core1/sensor_core1.cpp`, `src/core1/sensor_core1.h`, `src/fusion/eskf_runner.cpp`, `src/fusion/eskf_runner.h`, `src/main.cpp`, `src/safety/fault_protection.cpp`, `src/safety/fault_protection.h`, `src/shared_state.cpp`, `standards/STANDARDS_AUDIT.md`, `tools/spin/README.md`, `tools/spin/run_stage_o_ao_spin.sh`
