...
**`-Wshadow`/`-Wfloat-equal` gate-coverage hole (repo-owner-prompted positive control).** Proving the `-Werror` compiler gate was actually *live* (not assuming "0 warnings = clean") surfaced that `CMakeLists.txt:603-604`'s per-file `-O2` override silently dropped `-Wshadow`/`-Wfloat-equal` from `eskf.cpp` + `eskf_codegen.cpp` — `set_source_files_properties(COMPILE_OPTIONS)` *replaces*, not appends — leaving two flight-critical fusion files ungated since the override was added. A compile-command sweep of all 75 RocketChip-authored `src/` files confirmed these were the only two holes (73 already gated; 91 vendored SDK files correctly excluded). Re-added the flags; the now-live gate caught 8 `-Wfloat-equal` hits in eskf.cpp — all compile-time `static_assert` codegen-drift guards (exact named-constexpr identity, not a runtime hazard), suppressed for that block only via `#pragma GCC diagnostic`. Positive-controlled: planted a shadow + float== in eskf.cpp → build failed both `-Werror` diagnostics → reverted → clean. Verified: WSL `-Werror` rebuild clean (eskf files now gated), host ctest 857/857, bench_sim 2/2 PASS (vehicle flight v0.16.0 (kmenu), COM7, sensors GO). (CMakeLists.txt, src/fusion/eskf.cpp)

**P10-7 return-value canary closed via `[[nodiscard]]` contracts (council: JPL / Professor / Cubesat).** Populating the long-empty `bugprone-unused-return-value.CheckedFunctions` surfaced 5 unchecked calls — all best-effort `i2c_bus_recover()` (×4) + one GPS drain read. Rather than `NOLINT` the call sites (a deviation with no register, lost in the static), the panel moved the contract onto the API: `[[nodiscard]]` on the i2c_bus data path + `init`/`probe` and the gps status getters (compiler-enforced under `-Werror` in every TU — stronger than the linter list and not lost at a call site), **not** on `recover`/`reset` (best-effort by contract → the 4 calls correctly stop being findings). The drain is `(void)`'d. clang-tidy `CheckedFunctions` shrinks to the one vendored function we cannot annotate (`flash_safe_execute`). Reserved-id `__StackBottom`/`__StackOneBottom` consolidated into one boundary header (`include/rocketchip/linker_symbols.h`) — one documented suppression, not 3 scattered. **Gate-wiring:** new `scripts/audit/full_tree_clang_tidy.sh` is the single source of truth for the gated checks; the pre-commit hook (Gate 2) and `SESSION_CHECKLIST` item 17 both call it (fixes the LL-40 dual-hardcode). Also adds `ParameterThreshold=6` (JPL-25). Positive-controlled: `[[nodiscard]]` fires (`-Werror=unused-result` on a planted discard); the script catches a planted reserved-id and exits 1; full-tree gate clean (75/75 coverage). Verified: WSL target + host build clean, host ctest 857/857, bench_sim 2/2 PASS (vehicle flight v0.16.0 (kmenu), COM7, sensors GO); no codegen/behavioral change (compile-time contracts + `(void)` + decl consolidation). (.clang-tidy, src/drivers/{i2c_bus,gps_uart,gps_pa1010d}.h, src/main.cpp, src/safety/*, include/rocketchip/linker_symbols.h, scripts/audit/full_tree_clang_tidy.sh, scripts/hooks/pre-commit, docs/agents/SESSION_CHECKLIST.md)
...
- CCSDS doc: CLCW bit layout corrected to 232.0-B-4 (layout) + 232.1-B-2 (COP-1 procedures); PLTU/ASM ownership corrected to 211.2-B-3; AX5043 language corrected (suppressed-carrier BPSK, not residual-carrier 60° PM); SX1276 datasheet updated to Rev. 7 (May 2020); early optimistic §2.3 language toned down with pointer to the later corrected §2.5 analysis; duplicate/scramble tail after §2.5 excised for document hygiene.
