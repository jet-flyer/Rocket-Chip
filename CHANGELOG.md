...
### 2026-06-22-001 | Grok 4.3 | docs, starcom

**Condensation of all CCSDS preliminary documents into canonical DESIGN.md.** Merged feat/condense-starcom-ccsds-prelim-20260622 to main. Includes agreement/conflict/gaps table (from comparison + design_record), §0 scope verbatim, FOP-1/FARM-1 state machine summary + external 232.1-B-2 REF note, D-1..D-5, unique data excerpts (PLCW 7-field, USLP header, PIO prior-art, <50km impact, etc.), modern naming convention (no archaic I-prefix per direction), pointer to detailed Claude council Round 2 verdict in design_record_claude.md for D-2 sans-I/O. Zero substantive loss; historical docs untouched; only starcom/ changes. Updated starcom/CHANGELOG.md, STATUS.md, README.md, manifests, scripts. Verified: pure-software change, host ctest 857/857, capture evidence in SCRATCH, no HW. (starcom/, CHANGELOG.md)
