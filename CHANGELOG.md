...
Bierman host/flight consolidation (`0382b17` + follow-on tests): host `rc_fusion` matches flight Bierman UD measurement path; Joseph dual path and `rc_fusion_bierman` removed; dense-P contract (`sync_dense_covariance`, per-instance UD); replay goldens rebaselined; mat `joseph_update*` deleted. Rule-7 fixes: multi-ESKF shared UD, factorize-fail→NaN on long mag, Bierman D floors. Thornton predict documented as RP2350 cycle-budget shelve (not impossible) with revisit triggers. Host regressions: `DualESKFIndependentUD`, `LongMagAidingStaysFinite`. Verified: host ctest (incl. new tests); vehicle flash `compare-sections` match; banner `flight-0382b17`; bench_sim 2/2 PASS COM7. Note: SWD load may not USB re-enum — use banner hash + section verify, not audio. CHANGELOG deferred until owner-approved; this entry covers the landed work.
