...
**Session checklist restructured to aviation phase model.** Rewrote the "How to use" guidance + added explicit "Phase Entry Points" list in `docs/agents/SESSION_CHECKLIST.md`. Changed from "always start at the absolute top of the whole document" to dedicated phase sections with clear entry points. Normal Session End and Milestone are now properly nested (every milestone close includes a normal session end; you begin at the relevant header). Per Commit / Per Push remain inherited baselines. This codifies the intended scoping so all agents consistently apply only the checklists for the actual phase (normal wrap-up vs. stage close) without re-running unrelated start-of-session items. (docs/agents/SESSION_CHECKLIST.md, CHANGELOG.md)
