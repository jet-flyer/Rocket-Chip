...
**Master Standards Audit 2026-05-07 — Phases A through 8 partial (Category 1 closed, Category 2 closed, Category 3 partial).** Continuous audit work spanning multiple sessions; this entry covers everything from Phase A.2 forward to Phase 8 Category 3 R-2/R-5 deferred. The next session resumes at R-6c (FP-1 template dispatch). The dated audit report `docs/audits/MASTER_STANDARDS_AUDIT_2026-05-07.md` holds the full per-step record; `docs/PROBLEM_REPORTS.md` is the project-wide PR tracker that this audit established and populated.

**Phase A.2 → R-6 → R-6b (file rename + accepted-deviation re-evaluation + standards-precedence rule codified).** `STANDARDS_DEVIATIONS.md` renamed to `ACCEPTED_STANDARDS_DEVIATIONS.md` to disambiguate "accepted permanent deviations" from "fresh audit findings awaiting disposition." 32 references updated across 11 state-of-system docs in the same commit per the trigger-driven-doc-edits rule. Phase A.2 re-evaluated every Active deviation row. R-6 corrected a JSF AV Rule 170 misreading proliferation (the rule prohibits >2 levels of pointer indirection, not function pointers — JSF Rule 176 only requires typedef-declared function pointers, which our code does). R-6b caught a deeper failure mode: function pointers ARE prohibited, but by **Power of 10 Rule 9** (Holzmann, 2006), not JSF. FP-1 reinstated under the correct rule. New `CODING_STANDARDS.md` Foundation section captures the standards-precedence rule (chronological with explicit override carve-outs). LL Entry 37 documents the three-step rule-citation discipline that came out of this near-miss.

**Phase B → Phase 0 → Phases 1-7 (baseline scripted sweeps + dispositions + manual phases).** `AUDIT_GUIDANCE.md` Appendix B inlined ~3 pages of reference material (P10 rule list, Koopman 5-rule embedded review, FMEA worksheet columns, manual-audit walking procedure). Phase 1 ran every Category 2 script end-to-end with positive-control verdicts captured in `logs/audit-2026-05-07/`. Phase 3 satisfied PRE_FLIGHT_CHECKLIST with 3-boot reseat protocol. Phase 4 ran the FMEA-lite review (6/6 PASS) + Koopman pass (15/15 PASS) + enhanced_fault_injection harness rewrite (1/4 PASS at audit time, the gaps became R-9a/b/c). Phase 5 verified errata E2/E9/E11/E12 + stack-usage ≤200 bytes empirically. Phase 6 ran all 4 active SPIN models — `SPIN_OK_26` (26/26 LTL properties verify clean). Phase 7 walked requirements traceability 12/12 CONFIRMED with raw-quote evidence.

**Phase 8 ordering framework.** `AUDIT_GUIDANCE.md` Appendix C codifies the 4-category remediation ordering (Gate Integrity → Lower-rate Safety-Critical → Higher-rate Drivers → Process/Doc) per DO-178C "fix gates before findings, fix verification before remediation" pattern. PROBLEM_REPORTS.md established as the project-wide PR tracker per DO-178C problem-report concept. `.claude/SESSION_CHECKLIST.md` Per-Commit gets new item 5a (Change Impact Analysis for non-trivial changes). `HW_GATE_DISCIPLINE.md` gets Rule 6 (local-commit verification ≠ audit-suite regression credit) + Rule 7 (surfaced-bugs in scope for the PR that surfaced them).

**Phase 8 Category 1 — Gate Integrity (closed 2026-05-12).** Five PRs: P8-SPIN-A extends the SPIN gate to all 4 active models (was gating only `rocketchip_fd_min.pml`; now `SPIN_OK_26` covers all 4); P8-SPIN-B deletes a zero-byte `rocketchip_fd_pp.pml` stub. R-10a fixes the `analyze_stack_usage.sh` parser bug (was reporting `0 KiB` for everything; now reports honest `5 KiB`). R-9a + R-9b add `fault_force_launch_abort()` and `fault_force_radio_dropout()` (closing the enhanced_fault_injection PARTIAL → PASS for those scenarios with firmware-side positive-control signals observed). Cat 1 → Cat 2 transition gate signed.

**Phase 8 Category 2 — R-3 hardfault handler refactor (closed 2026-05-12).** Replaced the halt-forever fault handler with capture-state-then-reset architecture. New `src/safety/crash_record.{h,cpp}` (preserved-SRAM at 0x20000110); `health_monitor.cpp` reads + reports on next boot. Two MPU bugs surfaced and fixed in the same commit per Rule 7: AP encoding corrected from 0b00 (RW Privileged — wrong) to 0b10 (no-access for all), MEMFAULTENA enabled in SCB->SHCSR (was unset, MPU faults were escalating to HardFault instead of invoking MemManage). Verified on bench: crash record captured at 0x20000110, post-reset banner reports `[HEALTH] prior-boot hardfault (MemManage) cfsr=0x82 lr=0x20081000`. Amended (commit `0b0e2f5`) to inline-only handler (zero function calls) after fault-on-fault lockup discovered during 3-boot reseat verification: a C-level function call inside the handler crossed the MPU stack guard and produced PC=0xeffffffe lockup. `docs/decisions/FAULT_HANDLER_DESIGN.md` captures the design rationale (Memfault Cortex-M HardFault guide + ARMv8-M ARM § "Lockups in handler mode").

**Phase 8 Category 3 partial:**
- **R-1 (verified):** Bounded the Core 1 boot-wait loop on the vehicle path with `if constexpr` role check + `crash_record_capture(kCrashReasonCore1BootWait)` on timeout. Closes BM-2 (P10 Rule 2 inverted-rule exemption check — boot wait now has a finite upper bound).
- **R-9c (verified):** `health_monitor.cpp` `log_secondary_transitions()` emits log-on-change for Radio/Flash/Watchdog/PIO/Core1 secondary health bits (previously only primary bits had transition logging).
- **R-5 (deferred to dedicated session):** Full `<stdio.h>` removal from `src/` (no carve-outs by classification — user direction 2026-05-12 to retire the Ground-stdio-relaxation rationale). The complexity warrants a dedicated session. **Proliferation prevention is ACTIVE NOW**: `scripts/hooks/pre-commit` Gate 1 + `scripts/hooks/stdio_allowlist.txt` (18 currently-using files) block any new `<stdio.h>` include in `src/*.cpp`. `docs/decisions/STDIO_REPLACEMENT_PLAN.md` captures the full design (rc_log + ETL + dispatch channel) for the dedicated session. `docs/STDIO_TO_RC_LOG_MIGRATION_GUIDE.md` is the developer cheat-sheet for new code today.
- **R-2 (absorbed into R-5):** GPS PMTK `snprintf` → const arrays scope-investigation surfaced that taking `gps_pa1010d.cpp` fully off the allowlist requires migrating one residual `snprintf` in `gps_pa1010d_get_debug_status()` that needs `etl::format` to migrate cleanly. Rather than vendor ETL just for that one callsite (out of R-2 scope) or hand-roll a one-off int-formatter (re-inventing R-5's plan), R-2 bumps onto the R-5 dedicated session. Council review (JPL + ArduPilot + Professor, 2026-05-13 — work attribution date follows wall-clock-when-done, even though the surrounding audit work dates 2026-05-12) consensus: Approach B (surgical cleanup — delete dead `set_rate` functions, collapse `negotiate_baud` to `void`, remove `send_command` from public headers, replace 8 of 9 snprintf with `constexpr` wire arrays, migrate the 9th to `etl::format`). 3 council amendments preserved for the dedicated session: constexpr-checksum verification via `static_assert`, byte-on-wire HW gate per HW_GATE Rule 1, header-API removal documented. Pre-computed checksums for the 4 unique PMTK sentences: PMTK314=0x29, PMTK220-1000=0x1F, PMTK220-100=0x2F, PMTK251-57600=0x2C.

**Additional architecture work captured during the audit:**
- `AGENT_WHITEBOARD.md` "In-flight fault recovery architecture" entry frames 5 questions for a dedicated future session: (1) is reset the right primitive for in-flight faults (datum loss problem)? (2) persistence of recovery-critical state across reboots; (3) PIO watchdog as the independent check + safe-mode trigger; (4) IEC 61508 HFT=0 framing for single-MCU systems; (5) proper aerospace-grade fault handling = multi-MCU board work (Gemini and Titan options; Pegasus FC is a separate side-project that chooses its own architecture).
- **Surfaced limitation — Core 1 SIO_FIFO_IRQ post-AIRCR-reset wedge.** AIRCR.SYSRESETREQ on RP2350 is processor-only (datasheet §7.3.1); doesn't reset Core 1's NVIC. Post-reset, the pending SIO FIFO IRQ fires before any handler is installed and vectors to `isr_invalid`. Several fix attempts didn't work (drain SIO from handler, drain SIO + clear NVIC pending in core1_entry, no-op handler installation). Three candidate paths logged for the dedicated session: modify SDK Core 1 boot to clear NVIC pending earlier, use a chip-level reset (PSM-based), or sidestep entirely via PIO watchdog + safe-mode-in-place.
- `docs/PROJECT_STATUS.md` re-adds "Side Projects & Future Product Lines" section (Pegasus FC, Fruit Jam GS, OpenMCT, Gemini dual-MCU, Titan multi-MCU eval).
- `ACCEPTED_STANDARDS_DEVIATIONS.md` adds TP-1 (Pico SDK `spi_set_format` cognitive complexity 31 vs threshold 25 — vendored code we don't fork, pre-commit hook correctly filters to staged `src/*.cpp` only).

**LL Entry 37 added.** Three-step rule-citation discipline (verify wording against primary source, verify the right standard in the precedence chain, apply precedence rule explicitly when standards disagree) — driven by the R-6 → R-6b near-miss.

Verified: pure-software change for all R-5 / R-2 / R-9c / R-1 / R-3 commits (commit messages cite per-commit positive-control signals per HW_GATE Rule 3). Host ctest 788/788 PASS at session end. Target-build parity (4 tiers) verified at the R-3 commit boundary (`e4d222a`); no source changes since.

Files (representative): `docs/PROBLEM_REPORTS.md`, `docs/decisions/STDIO_REPLACEMENT_PLAN.md`, `docs/decisions/FAULT_HANDLER_DESIGN.md`, `docs/STDIO_TO_RC_LOG_MIGRATION_GUIDE.md`, `docs/audits/MASTER_STANDARDS_AUDIT_2026-05-07.md`, `docs/PROJECT_STATUS.md`, `standards/AUDIT_GUIDANCE.md`, `standards/CODING_STANDARDS.md`, `standards/HW_GATE_DISCIPLINE.md`, `standards/ACCEPTED_STANDARDS_DEVIATIONS.md`, `.claude/SESSION_CHECKLIST.md`, `.claude/LESSONS_LEARNED.md`, `AGENT_WHITEBOARD.md`, `src/safety/fault_protection.cpp`, `src/safety/crash_record.{h,cpp}`, `src/safety/health_monitor.cpp`, `src/core1/sensor_core1.cpp`, `src/active_objects/ao_rf_manager.{h,cpp}`, `src/dev/fault_inject.cpp`, `scripts/hooks/pre-commit`, `scripts/hooks/stdio_allowlist.txt`, `tools/spin/run_stage_o_ao_spin.sh`, `scripts/analyze_stack_usage.sh`, `scripts/enhanced_fault_injection.py`, `CMakeLists.txt`.
