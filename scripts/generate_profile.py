...
def generate_header(name, values, qr_data, ramp_steps, cfg_path, symbol,
    ...
    try:
        cfg_display = Path(cfg_path).resolve().relative_to(repo_root).as_posix()
    except ValueError:
        cfg_display = Path(cfg_path).as_posix()

    profile_id = PROFILE_IDS.get(name, 'ProfileId::kRocket')

    lines = []
    lines.append('// SPDX-License-Identifier: GPL-3.0-or-later')
    lines.append('// Copyright (c) 2025-2026 Rocket Chip Project')
    lines.append('// AUTO-GENERATED by scripts/generate_profile.py')
    lines.append(f'// GENERATED_FROM: {cfg_display} (sha256: {src_hash})')
    lines.append('//')
    lines.append('// Do not edit this file directly.')
    lines.append(f'// Edit {cfg_display} and re-run the generator.')
    lines.append('')
    # Include guard (JSF AV — standard #ifndef guard, not non-standard #pragma once).
    # Derived from the output filename so each generated header is unique.
    guard = 'ROCKETCHIP_' + os.path.splitext(os.path.basename(output_path))[0].upper() + '_H'
    ...
    # has_default_location: derived — true iff a real default location is configured
    # (replaces the old 0,0 float-equality sentinel; JSF AV 202 / no float ==).
