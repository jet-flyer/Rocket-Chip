...
**Phase:** increment 16 generic SPI/GPIO radio port in. Product `0.16.0-dev`. **Sans-I/O data-link core cut (0–14); host file/UDP (15); generic bus (16).** Next is 17 (PIO port). ASan not run on this MinGW (no libasan). No tag. No SC-NNN.

## Next

IVP increment 17 (PIO port). Sequence through 25 is in `docs/IVP.md`. RC integration (IVP 20–22) when scheduled. Host dissect demo is a Starcom WB sidetrack. CFDP (727.0) is wanted post-mission offload, not 0–25.
...
- None for increment 16. This MinGW g++ has no libasan/libubsan; sanitizer *run* waits for increment 24 on Clang/Linux.
