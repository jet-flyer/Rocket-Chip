...
**Phase:** increment 20 RC host consumer link in. Product `0.19.0-dev` (no Starcom version bump for an RC sitting). Next is 21 (Pico+AO — Buzz). ASan not run on this MinGW (no libasan). No tag. No SC-NNN.

## Next

IVP increment 21 (Pico link + first AO byte pump). Sequence through 25 is in `docs/IVP.md`. Not 22 (COP replace) and not air-path STOP-GAP strip unless scheduled. Host dissect demo is a Starcom WB sidetrack. CFDP (727.0) is wanted post-mission offload, not 0–25.
...
- Increment 20: RC host `add_subdirectory(starcom)` when `ROCKETCHIP_USE_STARCOM=ON`; `Starcom::starcom` linked from host PLTU round-trip / STOP-GAP reject-as-PLTU. Nested `STARCOM_BUILD_TESTS=OFF`. Pico link is 21. Product stays `0.19.0-dev`.
