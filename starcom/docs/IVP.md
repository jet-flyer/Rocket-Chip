...
Landed on `grok/sc-dev`. RC `add_subdirectory(starcom)` when `ROCKETCHIP_USE_STARCOM=ON` (host `BUILD_TESTS`) with `STARCOM_BUILD_TESTS=OFF` for the nested build. Host tests see `starcom/version.hpp`, round-trip a PLTU (v3-header-only and nav SDU 18+N), and reject STOP-GAP 54 B frames as PLTUs (`decode_pltu` → `bad_asm`). Default OFF does not add_subdirectory and does not link Starcom. Dependency remains **RC → Starcom**. Pico `target_link_libraries(rocketchip Starcom::starcom)` is increment 21. Not a firmware-pin sitting. Product stays `0.19.0-dev`.
