...
| PLTU repeater (bent-pipe and/or buffered) | Not a Blue Book product. Related: 211.2 C&S check; 133.0-B-2 §2.4 (subnetwork storage/forwarding assumed, not an SPP procedure) | In scope (IVP 7 bent-pipe; 12 buffered) | `repeat_pltu` (`tests/unit/test_pltu.cpp`). Bit-exact after `decode_pltu`. Buffered / dedup is increment 12. No COP on this path. |
...
| Version-4 / USLP transfer frame in the same PLTU | 732.1-B-3 Fig 4-1 | In scope (MVP; remainder IVP 9) | Non-truncated header + TFDF (`tests/unit/test_uslp.cpp`). Truncated header / Insert Zone / FECF are increment 9. Not nested in the V-3 data field. No SC-NNN. |
...
| COP-P procedures (FOP-P / FARM-P) | 211.0-B-6 §7 | In scope (MVP; remainder IVP 11, 13) | Tables + `CoppEndpoint` (`tests/unit/test_copp.cpp`). SET V(R) persistent activity (7.2.3.2) is MAC — increment 13. COP-P on a USLP VC is increment 11. No SC-NNN. |
| COP-1 procedures (FOP-1 / FARM-1) | 232.1-B-2 | In scope (MVP; remainder IVP 10) | FARM-1 Table 6-1 + FOP-1 E23/AD/E8 (`tests/unit/test_cop1.cpp`). S4/S5 BC-init and remaining FOP-1 events are increment 10. No SC-NNN. |
