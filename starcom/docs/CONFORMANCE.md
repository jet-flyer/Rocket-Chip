...
| PLTU repeater (bent-pipe and/or buffered) | Not a Blue Book product. Related: 211.2 C&S check; 133.0-B-2 §2.4 (subnetwork storage/forwarding assumed, not an SPP procedure) | In scope (MVP bent-pipe) | `repeat_pltu` (`tests/unit/test_pltu.cpp`). Bit-exact after `decode_pltu`. Buffered grade / dedup still whiteboard. No COP on this path. |
