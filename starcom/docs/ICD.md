...
Result<std::size_t> encodeSetPhy(std::span<std::byte>, MacPhyParams const&, bool transmitter);
Result<MacPhyParams> decodeSetPhy(std::span<const std::byte>, bool* transmitter_out = nullptr);
Result<std::size_t> encodeSetControl(std::span<std::byte>, MacControlParams const&);
Result<MacControlParams> decodeSetControl(std::span<const std::byte>);
Result<std::size_t> encodeSetPlExt(std::span<std::byte>, MacPlExt const&);
Result<MacPlExt> decodeSetPlExt(std::span<const std::byte>);
void macLoadHailCommValue(MacSession&, MacCommValue const&);
void macLoadPendingCommValue(MacSession&, MacCommValue const&);
