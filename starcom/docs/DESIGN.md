...
**Best-effort rule of thumb** (not a hard red line): only when it advances performance or product features, especially if the work later transfers to Full. Do not add it just to have it. A lot of current radio Best-effort is Rocket-Chip-specific even when sequestered; RC pins and AO stay out of `starcom::ccsds`.
