...
**Note on full state tables:** The in-corpus source (design_record_claude.md §2.7) provides the state-name summary table above. The complete 46-event transition details for FOP-1 (Table 5-1) / FARM-1 (Table 6-1) live in the external CCSDS 232.1-B-2 Blue Book itself and are preserved by reference (REF), not duplicated here. This satisfies no-loss without inventing source text absent from the six preliminary documents.
