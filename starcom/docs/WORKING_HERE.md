...
**Identifiers:** public verbs are house camelBack (`decodePltu`) from IVP 23. The closed `Error` ICD (`uslp_truncated`, `bad_asm`, …) stays — cFS keeps existing common-use names; F´ applies house style when a type is *born*, it does not retrofit a rename of a closed set. New identifiers are house camelBack when written. Book field names live in comments / CONFORMANCE. JSF 151: use the `k*` that already names the field (`kTfvnV3`, `kFopPSentCap`, `kIdleApid`); do not add `MacSs::ss0`-style wrappers or a `1u`→`1U` sitting. No Starcom accepted-deviation row. `#pragma once` is the existing project-wide exception, not a Starcom row.
