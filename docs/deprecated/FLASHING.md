...
**Not the flashing procedure.** Current source of truth: `docs/FLASHING.md`.

Kept because the 2026-09-02 rewrite may have dropped a caveat. Recipes here
that use OpenOCD `program`, GDB `load`, or `monitor reset halt` latch
STEMMA slaves (Analog AN-686). USB unplug is recovery, not a step.
