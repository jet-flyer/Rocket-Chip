...
### 1. ESKF Alternative Covariance Paths / Bierman Fallback (Larger Impact) — **CLOSED 2026-07-09**
- **Files:** `src/fusion/eskf.cpp` / `eskf.h`, `test/CMakeLists.txt` (collapsed `rc_fusion_bierman`), mat `joseph_update*` removed, host replay refs rebaselined.
- **Description:** Bierman/UD chosen (commit `cbdeb60`, 2026-02). Joseph dual path retained behind `#ifdef` for A/B until host parity.
- **Closure:** Host `rc_fusion` matched flight Bierman; dense-P contract (`sync_dense_covariance` / per-instance UD); Joseph `scalar_kalman_update` + `#else` forks deleted; dual host fusion lib collapsed. NASA/TP-2018-219822 UD practice documented in ADVANCED_SETTINGS. Surfaced-issue fixes (Rule 7): multi-ESKF UD was file-static; factorize-fail still marked UD → NaN cascade on long mag replay; D floors in Bierman.
- **Preserves:** Bierman path, 24-state ESKF, inhibits, codegen FPFT predict.
- **References:** `docs/decisions/ESKF/ESKF_RESEARCH_SUMMARY.md`, `docs/benchmarks/UD_BENCHMARK_RESULTS.md` (CHANGELOG entry deferred until owner approves consolidation).
