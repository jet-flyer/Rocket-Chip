...
|---|---|---|---|---|---|
| F-2026-05-13-001 | 1.3 | No synthetic staged-diff fixture exists to validate the pre-commit hook's gate matrix at audit time. | Minor | REMEDIATED | `scripts/audit/pre_commit_fixture_test.py` (commit `2eb2cd0`, 2026-05-13 follow-up session). 30-row fixture exercises `scripts/ci/pre_commit_matrix.py` against known-good / known-bad / no-fire / edge cases including substring-not-prefix and rc_os* overlap. All 30 rows PASS locally. AUDIT_GUIDANCE Tier 1.3 wired to name the script as the audit-time mechanism. |
| F-2026-05-13-002 | 1.4 | No maintained PASS-token inventory for bench_sim's deleted-regex check. Manual walk this cycle. | Minor | REMEDIATED | `scripts/audit/list_bench_sim_pass_tokens.py` (commit `b80fea8`, 2026-05-13 follow-up session). Scans src/ for emitted [FD] / VERDICT / RegVersion / [Health] / [N/A ] / [FAULT] / [ESKF] tokens; cross-references against the 4 bench_sim + 3 station_bench_sim regex constants. Output: `logs/bench_sim_pass_tokens.txt`. 10/11 token classes match firmware emissions; 1 empty class ([ESKF]) is acknowledged doc-vs-implementation drift in the class Scope note, not gated. AUDIT_GUIDANCE Tier 1.4(b) wired to name the script. |
| F-2026-05-13-003 | 2.3 | No `scripts/check_toolchain_drift.sh` to mechanically pull upstream versions per component. Audit-time check is manual + partial. | Minor | REMEDIATED | `scripts/audit/check_toolchain_drift.py` (commit `fcb4cd8`, 2026-05-13 follow-up session). Mechanically pulls upstream-latest for Pico SDK, picotool, OpenOCD Pi-fork branch HEAD, Pico Probe firmware, CMake; reads project pins from CMakeLists.txt + local installs from `~/.pico-sdk/<component>/`. Graceful network-failure degradation. Output: `logs/toolchain_drift.txt`. Local run on 2026-05-13: Pico SDK + picotool clean; OpenOCD/debugprobe/CMake drift matches rationale documented in `TOOLCHAIN_VERSION_AUDIT_2026-04-27.md` — no NEW drift detected. AUDIT_GUIDANCE Tier 2.3 wired to name the script. |
...
### Retroactive amendment: F-001/F-002/F-003 DEFER → REMEDIATED (2026-05-13 follow-up session)

The audit cycle (2026-05-13) closed with F-2026-05-13-001/002/003 DEFERRED to the audit-infrastructure queue and the user-spelled-out expectation that a follow-up session would build the three scripts. A focused script-tooling session on 2026-05-13 (the same day as the audit close, but in a fresh Claude context) built all three:

- `scripts/audit/pre_commit_fixture_test.py` — F-001 (commit `2eb2cd0`)
- `scripts/audit/list_bench_sim_pass_tokens.py` — F-002 (commit `b80fea8`)
- `scripts/audit/check_toolchain_drift.py` — F-003 (commit `fcb4cd8`)
