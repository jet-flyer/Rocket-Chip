...
**Progress (2026-08-24):** Phase 0–2 closed. Phase 3 owner do-now on `main`. Overlay paper, QMI, remaining PSRAM, dashboard display lies, log-ring, GPS UART, cross-core publication / fail-open closed. **Next: Phantom / missing symbols.** Per remaining item: summary + suggestion before `src/`. Keep worktree `grok/l2p5-disposition`.
