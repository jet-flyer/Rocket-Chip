...
> **Errata (2026-06-25, L2-P5 Phase E — read alongside the body):**
> - **`rc_log` is `void`** → it is **N/A to return-value checking**. Where §2/§4c/§7 list it among the return-value-canary "uncovered" functions (`i2c_bus_*`/`flash_safe_execute`/`gps_*`/`rc_log`), strike `rc_log`. The return-checkable log fns are `rc_snprintf`/`strbuf_overflowed`. (`flash_safe_execute` **is** `int` → correctly return-checkable.)
> - **`#pragma once` = 3 headers, not 1** (`fault_protection.h`, `mission_profile_data.h`, `shared_state.h`) where §4a/§7 say only `shared_state.h` / "32/33 compliant." **All 3 were remediated to `#ifndef` guards in the 2026-06-24 batch** → now 0 (`#pragma once` no longer present in authored headers).
> - **The P10-7 return-value CANARY is now CLOSED** (Phase B, 2026-06-24): project APIs carry `[[nodiscard]]` under `-Werror`; `.clang-tidy CheckedFunctions = flash_safe_execute` (the one vendored fn). The "uncovered" framing in §2/§4c/§7 is historical (true at 2026-06-17 triage time).
> - **Gate-infra dual-hardcode (LL-40) fixed**: the gated-check list now has one source (`scripts/audit/full_tree_clang_tidy.sh`), called by both the pre-commit hook and `SESSION_CHECKLIST` item 17.
> - **§459 naming over-claim RESOLVED 2026-06-25** (see that line) — recorded in `CODING_STANDARDS.md` (Worked-consolidation bullet + reworded checklist) + `QP_APPLICATION_GUIDE.md` §6.5.
...
- **JSF 151 (Split, low):** literal-vs-ignore-list = Det-by-reference (curated set: `0;1;2;3;4;8;16;32;64;100;255;256;1000` + floats + powers-of-2 + bitfield widths) via `readability-magic-numbers`; whether a flagged literal *should* be named = Grey. Honest-narrow: a value in the curated set passes silently even when contextually magic. **L2-P5 (2026-07-28):** not a semantic-walk lens — mechanical gate first; human residual only when dispositioning tidy hits (named + sourced non-obvious values required unless impossible/highly impractical after review; flag ignore-list hideouts). Notes on the check: `.clang-tidy` MAGIC NUMBERS block + `CODING_STANDARDS.md` "No arbitrary numerical values" / pre-commit JSF 151.
...
- **JSF 175 (Grey, critical):** mandates plain `0`; project ENABLES `modernize-use-nullptr` which rewrites both `0` and `NULL` to `nullptr` — the **inverse**. Any audit citing AV 175 as "enforced/passed" over-claims. Converts to Det-by-reference once the supersession is written. **[RESOLVED 2026-06-23: supersession recorded in `CODING_STANDARDS.md` Foundation → Worked consolidation decisions (Null pointer); codebase verified uniformly `nullptr` (only `NULL` in `src/` is a doc comment); QP/C `(void *)0` sender idiom converted to `nullptr`.]**
- **JSF 182 (Grey, critical):** bans casting to/from pointers; the RP2350 bare-metal target *structurally* requires it (MPU setup, HW registers, CFSR reads) and `pro-type-reinterpret-cast` is SKIPPED for exactly this reason. Standing deviation that must be logged, not silently passed. Converts to Det-by-reference once an accepted-pointer-cast list is authored. **[ERRATUM 2026-06-23 (L2-P5 walk): the blanket "standing deviation" framing is too broad. The full `reinterpret_cast` inventory in `src/` was dispositioned per category against the rule's actual exceptions — literal/HW-address→pointer = **compliant** (Exception 2); struct→`uint8_t*` CRC + buffer→header overlay = **remediated to compliance** (Exception 1); only TWO native-code residuals are logged as accepted deviations — **CAST-1** (MPU ptr→int) + **CAST-2** (`evt_cast` QEvt downcast). `pro-type-reinterpret-cast` correctly stays off (it cannot distinguish the compliant Exception-1/2 forms from the residuals). See `ACCEPTED_STANDARDS_DEVIATIONS.md` → "JSF AV Rule 182" and `CODING_STANDARDS.md` Foundation → Worked consolidation decisions → Pointer casts.]**
...
- **JSF 45 / 51 / 52** (naming): `readability-identifier-naming` enforces camelBack/k-CamelCase/lower_case — which *contradict* JSF underscore-separation (45) / all-lowercase variables (51) / lowercase constants (52). `CODING_STANDARDS.md` cites the gate as "JSF AV Rule 50-53" compliance while it encodes a non-JSF scheme. The project convention legitimately supersedes JSF here; the *over-claim* is the doc text asserting JSF conformance. **Fix:** record the supersession explicitly. **[PARTIAL 2026-06-25 — drift corrected: the earlier "RESOLVED 2026-06-23: recorded in `CODING_STANDARDS.md` Worked-consolidation-decisions" claim was FALSE; it was never written there. The QP/AO-specific divergence (house naming vs Samek/QP `l_`/CamelCase state handlers) is now recorded in `docs/flight_director/QP_APPLICATION_GUIDE.md` §6.5 as a deliberate QP-convention decision — NOT a standards deviation (QP naming is non-binding per Quantum Leaps' own forkable style guide). **RESOLVED 2026-06-25 (repo-owner authorized):** the project-wide over-claim is fixed in `CODING_STANDARDS.md` — an "Identifier naming (JSF 45/51/52)" bullet added to Foundation → Worked consolidation decisions (peer to the Null-pointer bullet), and the Pre-Commit Checklist `:469` reworded to reference it instead of asserting "JSF AV 50-53" compliance. This closes the false-completion (this §459 note + CHANGELOG `2026-06-23-001` #2 had both claimed it done when only the nullptr bullet had actually landed).]**
- **JSF 175** (`use plain 0`): `modernize-use-nullptr` enforces the literal inverse (C++20). Honest disposition = deliberately-not-followed; an audit citing AV 175 as enforced over-claims. **[RESOLVED 2026-06-23: supersession recorded; see deviation log / CODING_STANDARDS Worked consolidation decisions (Null pointer).]**
- **JSF 182** (no casting to/from pointers): `pro-type-reinterpret-cast` SKIPPED; HW access structurally requires pointer casts. **[RESOLVED 2026-06-23: dispositioned per-category — compliant (Exc 2) / remediated (Exc 1) / two residuals logged (CAST-1, CAST-2). See deviation log; full erratum at §7 JSF-182.]**
