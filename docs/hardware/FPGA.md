...
Pinout systems named in the table (do not mix them): Teensy 4.0 0.600" 2x14; Feather / FeatherWing; Pico 0.1" header (pico2-ice is **not** this); Digilent PMOD 2x6 0.1" dual-row (0.1" **between** rows); UPduino custom dual-inline 24-pin (two single 0.1" rows, one per long edge).

### Master comparison

| Board | FPGA (+ MCU) | Form / pinout / connector | RAM/MAC | Verdict for Rocket Chip |
|-------|--------------|---------------------------|---------|-------------------------|
| **Forgix** | RP2354 + Efinix T8F49 (3x3 mm) | Teensy 4.0 0.600" 2x14. Not Feather. Adafruit 3200 mates mechanically; Feather SPI/I2C land on the FPGA, not the RP. Debug: Tag-Connect TC2030. | T8 ~123 kbit, 8x 18x18 MACs (little BRAM; package-size reason) | $50. On desk (unsoldered). Eval vehicle for T8-on-stamp. T8 on core stamp is **gated**: only if inhibit/MAC work PIO cannot do proves worth fabric/Efinity/power. Not for Starcom comms. T8 not on the base RC board. |
| **pico2-ice** | RP2350B + iCE40 UP5K | Pico-**width**, but **all** pins are Digilent PMOD 2x6 0.1" dual-row (ICE A, ICE B, RP-ICE, RP) — **not** Pico 0.1" header / Cowbell. Buy soldered vs not (no 90 vs vertical SKU); docs solder example is female 2x6 right-angle. J5 is 3-pin JST-SH BM03B-SRSS-TB Raspberry Pi Debug Probe SWD, **not** Qwiic. | UP5K 1 Mbit SPRAM + 120 kbit DPRAM, 8x MAC16 | $50. Not a Pico-header drop-in. |
| **RPGA Feather** | RP2040 + iCE5LP4K | Feather | iCE5LP4K: 3520 LUTs, few MAC16s, 80 kbit | **SKIP** (no RP2350 SKU). |
| **Oak Lattice FeatherWing** | iCE5LP4K (no MCU on the wing) | FeatherWing | Same iCE5LP4K | Most practical FPGA add-on on the current Feather stack. |
| **Galaxia** | Certus-NX LFD2NX-40 + Pico | Edge 2x6 90 deg plugs **are** Digilent PMOD 2x6 (80 I/O = ten ports: 8 I/O + 3.3 V + GND each) | 40k LUTs, 2.5 Mbit BRAM, 56 DSP 18x18 | ~$988 bench/space-dev. Not a rocket FC. |
| **UPduino v3.1** | iCE40 UP5K only (FTDI programmer; no RP) | Custom DIP stick ~2.2 cm x 6.2 cm. Two **single** 0.1" rows, one on each long edge (~Pico-like **row spacing**, not 0.1" dual-row). Not a Digilent PMOD 2x6 footprint. Tindie $36, quantity only; bag of two unsoldered 24-pin 0.1" headers. **No PMOD SKU on Tindie.** | UP5K 1 Mbit SPRAM + 120 kbit DPRAM, 8x MAC16 | FPGA-only eval, not Feather/Pico/Teensy drop-in. "PMOD compatible" is pin **order** on DIP holes, not a 2x6 header. |

### Forgix - RP2354 + T8

- Teensy 4.0 0.600" 2x14, ~$50. Not a Feather. On desk (unsoldered).
- Pins 11/12/13 and 18/19 on the T8 so GPS can live on RP UART1.
- LoRa SPI is FPGA-side (safe: not an RP radio).
- Adafruit 3200 Teensy-to-Feather adapter mates mechanically; Feather SPI/I2C land on the FPGA, not the RP.
...
- ~$50. Pico-width, but all pins are Digilent PMODs (ICE A, ICE B, RP-ICE, RP) — not Pico 0.1" headers.
- Not pin-compatible with Pico or Adafruit Pi Cowbell. PMOD is a separate expansion system.
- Buy option is soldered vs not (no 90 vs vertical SKU). The documented solder example is female 2x6 right-angle on the long edges.
...
- **RPGA Feather** (RP2040 + iCE5LP4K): Feather form. No RP2350 SKU. Skip.
- **Oak Lattice FeatherWing** (iCE5LP4K): FeatherWing add-on on an existing Feather. Most practical path to a small FPGA on the current Feather stack if not buying Forgix as the FC.
...
- ~$988
- 80 I/O via PMOD = ten Digilent 2x6 right-angle PMODs along the edge (8 I/O + 3.3 V + GND each)
- Also has an RPi Pico

### UPduino v3.1 (tinyVision.ai)

FPGA-only eval (UP5K + FTDI programmer). Not a Feather, Pico, or Teensy drop-in.
...
"PMOD compatible" on this board means the **pin order** on a region of those existing DIP holes, so you can jumper to a PMOD accessory. tinyVision's own shop (not Tindie) will solder a 6-pin PMOD for a fee. That is a shop service, not a Tindie option.
