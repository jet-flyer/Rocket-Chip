...
**Stage T COMPLETE (2026-04-22):** RF link manager architecture. Three batches landed:
...
**R-5 stdio removal CLOSED at Level-3 (2026-05-17):** Full `<stdio.h>` retirement from `src/*.cpp`. Cycle 2 of the four-cycle plan ([`parsed-soaring-popcorn.md`](../../.claude/plans/parsed-soaring-popcorn.md)). Project-side log infrastructure (`include/rocketchip/rc_log.h`) provides `rc::rc_log` (CDC log channel), `rc::rc_snprintf` (buffer-bound formatter), `rc::strbuf` (append-format helper) — one shared printf-style parser, three sinks. Council 2026-05-17 (NASA/JPL + ArduPilot + Professor + Cubesat, unanimous): parser-extract framing chosen over std::format-style alternative. Pre-commit Gate 1 upgraded to unconditional rejection of `<stdio.h>` in `src/*.cpp`; allowlist file deleted. Standards updates: `CODING_STANDARDS.md` Ground-tier stdio relaxation retired; `DEBUG_OUTPUT.md` examples switched to `rc::rc_log`. Audit-suite Level-3 regression evidence: 6/6 cold-restart bench_sim, 863/863 host ctest (+36 since pre-R-5 covering rc_snprintf + strbuf), SPIN_OK_31, station dashboard live + RF Link TRACK 100%, vehicle preflight 9/9 GO. PROBLEM_REPORTS R-5 + R-2 (absorbed) both moved to `closed`. Side-fixups landed alongside: GPS UART sticky-baud handler (CR1220 backup battery), HW_GATE_DISCIPLINE Rule 2 cable-reseat retired, ROCKETCHIP_OS doc drift fixes, dashboard hint string corrected.

**Next:** Cycle 3 (station GPS cold-boot fix) — R-5 prereq cleared; ready when a session picks it up. Cycle 4 (L2-P5 JSF AV walk + L2-P10 CLA-RBM re-collection) waits for Cycle 3. Stage 17 (Field Testing — DEFERRED, awaits airframe + launch window), Stage 18 (Field Tuning — awaits flight data). Remaining High priority: IVP-T13 LQ-adaptive retry + Stage T 95% first-try re-baseline — both explicitly deferred pending CCSDS command-layer rework. Station/vehicle pre-commit discipline items (classification-awareness, role-awareness, hook-portability into tracked scripts/hooks/) tracked in AGENT_WHITEBOARD.md. Build-system audit follow-ups: tier consolidation 4→2 COMPLETE 2026-05-13 via R-25-exec (single flight binary per role with runtime test-mode gating per `docs/decisions/BENCH_TIER_DEPRECATION_2026-05-13.md`); first toolchain-version audit run completed.
...
- **eyalroz/printf as rc_log/rc_snprintf exit-ramp candidate** — Current implementation is project-owned (`src/log/rc_log.cpp`, ~760 LOC) sharing parser between `rc_log()`, `rc_snprintf()`, and `strbuf_printf()`. Per 2026-05-17 stdio-replacement research, [eyalroz/printf](https://github.com/eyalroz/printf) is the closest IRL-vetted drop-in for our constraints (zero `<stdio.h>`, no libc `vsnprintf`, MIT licensed, 400+ tests, lint-clean, re-entrant). Adopting it would retire ~760 LOC of bespoke code in exchange for a tracked external dependency. Pre-adoption gate: float-rounding parity audit — eyalroz/printf uses simpler rounding than our libc-matching round-half-to-even, so an audit needs to confirm IMU/ESKF/baro flight-logging precision is unaffected. Worth revisiting if rc_log's drift surface becomes a maintenance burden, or if the float-rounding work informs upstreaming a fix to eyalroz/printf.
...
- `docs/agents/LESSONS_LEARNED.md` — Debugging journal
