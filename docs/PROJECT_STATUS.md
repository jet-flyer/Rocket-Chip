...
- **GPS 10Hz NMEA mode (experimental) + sticky-baud handling** — Theoretical max for the Feather UART GPS (PA1616D / MT3339) at 57600 baud is RMC+GGA+GSA at 10Hz, ~2.45× bandwidth headroom by math. 5Hz is the shipped production rate (matches ArduPilot default, all three sentences preserved at 4.9× headroom). The 10Hz attempt was never cleanly tested during R-5 Unit D part 2b because of a separate issue surfaced by the migration: **the MT3339 retains its baud setting across firmware power cycles**, so subsequent firmware boots fail `detect_gps_presence()` at 9600 (the module is still at 57600 from the previous flash's PMTK251). Symptom: `GPS: not detected` instead of streaming NMEA. Workarounds for the 10Hz investigation: (a) send PMTK104 cold-start command at boot to factory-reset the module before negotiate_baud, (b) try presence detection at 57600 first, fall back to 9600, (c) wire the MT3339 hardware reset pin to a Feather GPIO and toggle it at boot. Bandwidth math is fine; the sticky-baud + 10Hz transient issues are the actual investigation. Out of R-5 scope. Likely also needs a logic-analyzer trace on UART TX/RX for definitive verification.
