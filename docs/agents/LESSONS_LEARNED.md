...
**Purpose:** Process journal for this repo. Agents and humans re-read it so the same failure is not re-diagnosed from scratch.

**Format:** Historical entries. Typical shape is problem / symptoms / root cause / solution / prevention. **Time spent is optional** — many entries never had it; do not invent one. Existing entry bodies are append-only (typo or explicit SUPERSEDED header only). New lessons are new numbered entries.

**Holes 7–10, 14, 17–19:** FreeRTOS-era entries, archived on branch `AP_FreeRTOS` at the bare-metal pivot. Numbers stay unused here on purpose. Do not fill them. To read those bodies: `git show AP_FreeRTOS:docs/agents/LESSONS_LEARNED.md` (or the path that branch used).

**File order vs numbers:** Entry 34 was inserted after 35 in the file. Use the index below (numeric). Do not reorder historical bodies.
...
| # | Title |
|---|--------|
| 1 | Stack overflow from large local variables |
| 2 | Version strings for iterative debugging |
| 3 | BASEPRI blocking USB interrupts |
| 4 | Flash operations break USB |
| 5 | Use debug probe before manual BOOTSEL |
| 6 | WS2812/NeoPixel requires `begin()` |
| 7–10 | *(archived — `AP_FreeRTOS`)* |
| 11 | Prioritize debug probe over LED debugging |
| 12 | USB CDC init order |
| 13 | ICM-20948 I2C address is 0x69, not 0x68 |
| 14 | *(archived — `AP_FreeRTOS`)* |
| 15 | USB terminal connection affecting program state |
| 16 | PuTTY truncates initial USB CDC output |
| 17–19 | *(archived — `AP_FreeRTOS`)* |
| 20 | ~~PA1010D GPS I2C interference when probed~~ **SUPERSEDED 2026-09-05** |
| 21 | ~~ICM-20948 I2C master bank-switching race~~ **SUPERSEDED 2026-09-05** |
| 22 | ~~USB reconnect degrades Core 1 IMU rate~~ **SUPERSEDED 2026-09-05** |
| 23 | CLI I2C scan corrupts bus when Core 1 owns I2C |
| 24 | ~~PA1010D I2C 500 µs settling delay~~ **SUPERSEDED 2026-09-05** |
| 25 | ~~Picotool corrupts I2C bus~~ **SUPERSEDED 2026-04-22** |
| 26 | JSF AV 213 — arithmetic parentheses disabled |
| 27 | ~~"Codegen sensitivity" was picotool corruption~~ **REVISIT 2026-09-05** |
| 28 | ~~`i2c_bus_recover()` corrupts DW_apb on a live bus~~ **SUPERSEDED 2026-09-05** |
| 29 | ICM-20948 silent zero-output → ESKF divergence |
| 30 | RP2350 XIP cache thrashing / `.time_critical` |
| 31 | ~~`flash_safe_execute()` corrupts I2C + GPS lock~~ **SUPERSEDED 2026-09-05** |
| 32 | Blocking drivers vs QV cooperative contract |
| 33 | PIO GPIO init vs adjacent I2C pins |
| 34 | PC cooling fan turbulence → baro / ESKF P-growth |
| 35 | Stack-local QP events are use-after-free |
| 36 | Test artifact vs infrastructure — bench_sim silent rot |
| 37 | Rule-citation discipline (wording + precedence chain) |
| 38 | Code shows current-state; primary sources show possibility-space |
| 39 | `rc_log` idle drain vs Core 1 I2C |
| 40 | Pre-commit gate: categories not enumerations |
| 41 | RP2350B pads start isolated |
| 42 | PIO program lifecycle asymmetry |
| 43 | "Clean" from a static gate is negative evidence |
| 44 | Mass rename: clang-refactor (AST), not text replace |
...
4. **Session-start canary** — item 6 in `docs/agents/SESSION_CHECKLIST.md`. Run
   ...
   **SUPERSEDED 2026-09-07:** no boot / pre-edit canary. The land gate is the
   pre-commit hook (this entry's item 3). Ambiguous skip or hook-fail uses A/B
   recovery (flash last-known / `HEAD~`, `bench_sim`; flash the new build,
   `bench_sim`). See `standards/HW_GATE_DISCIPLINE.md` Rule 5. Checklist item 6
   is now the Per-Push station/vehicle build-parity check, not this canary.
