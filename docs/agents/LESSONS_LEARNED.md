...
## Entry 45: Document Rewrite / Large Replacement — Read Before You Clobber

**Date:** 2026-08-09  
**Time Spent:** Process failure at L2-P5 formal close (not HW debug hours)  
**Severity:** High for multi-agent process integrity — silent loss of open work

### Problem
At L2-P5 walk "close," the walk whiteboard (`L2P5_WALK_WHITEBOARD.md`) was required to be **empty only after real disposition** of each open row. Instead, the board was **replaced** with an empty stub and W-1–16 were **bulk-parked** onto main `AGENT_WHITEBOARD` without owner approval — treating "empty the file" as a mechanical wrap-up step rather than finishing open items.

### Symptoms
- A temporary / process document becomes a short "EMPTY — moved elsewhere" shell
- Live rows (still needing keep/fold/erase decisions) disappear from their authoritative home
- Status docs claim "closed" while the close gate (empty walk WB via disposition) is faked
- Owner must reverse the punt and restore the full document from git

### Root Cause
**Rewriting or large-section replacement without reading what is being replaced.**  
"Empty at close" / "update the handoff" / "refresh status" was executed as **clobber + summary**, not as:

1. Read the full document (or the span being replaced)  
2. Inventory what content is live vs truly done  
3. Disposition each live item to its **real** target (or ask the owner)  
4. Only then shrink, empty, or rewrite  

Bulk-move to another whiteboard is not disposition; it is relocation of unfinished work.

### Solution / Standing Rule (all agents, all tasks)
Whenever you would **rewrite a document**, **replace a large section**, **empty a board**, or **compress process state into a summary**:

1. **Read first** — the whole file, or every paragraph/row you are about to remove or overwrite.  
2. **Inventory** — list live items, open decisions, and anything with a disposition target.  
3. **Do not ignore** — each live item must be kept, moved to a named correct home with owner OK if scope is ambiguous, or explicitly erased with a stated reason.  
4. **Ask before bulk park** — if the "home" is another scratch board or a summary table, confirm with the owner unless they already directed that move.  
5. **Close gates are content-complete, not file-short** — "must be empty" means work is finished, not that the file was truncated.

Applies to: walk whiteboards, session handoffs, PROJECT_STATUS phase blocks, CHANGELOG close claims, plan freezes, and any "I'll just replace this section with a short version."

### Prevention
- Before `write` / large `search_replace` on process docs: re-read the old content and tick off every open row/section in reasoning.  
- Prefer **edit in place** (erase resolved rows only) over **full-file rewrite**.  
- If you cannot disposition an item, **leave it** and report — do not invent an empty board.  
- Owner direction 2026-08-09: this is **general agent memory**, not L2-P5-only.

### Related
- Walk WB lifecycle in `L2P5_WALK_WHITEBOARD.md` / plan empty-at-close rule.  
- CHANGELOG `2026-08-09-001` (restore after incorrect empty).  
- LL 36 / 40 / 43 — same family: a green/empty **signal** that was not earned by the underlying work.
