...
17d. **Graphify doc-graph: surface state, owner decides (prompt, never auto-run).** At milestone close, surface the graphify knowledge-graph status to the repo owner — specifically whether `graphify-out/.curate_flag.json` reports `stale_docs` (curated docs edited since their last semantic extraction) or `promote_sources` (new docs not yet curated). State the outcome explicitly per the per-item verification discipline ("Verifying 17d: .curate_flag reports ... Outcome: ..."). This is a **prompt, not a conditional action**: recommend the owner run `/graphify .` (the token-bearing LLM doc pass) only if this milestone actually changed docs that matter to the graph — a pure code-work milestone needs no semantic re-pass. The cheap path (`graphify update .` + `scripts/graphify_curate.py`, both no-LLM) already keeps code current and reconciles the curated shape on every rebuild; only the full re-extract is owner-gated. **Never auto-run `/graphify .`** — it is billed and owner-approval-gated. See `docs/tools/GRAPHIFY_USAGE.md` for the curate-filter / cache-staleness mechanism. Added 2026-06-28.
