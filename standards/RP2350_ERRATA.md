...
|---|---|---|---|---|---|---|
| 2026-08-21 | R-2 then R-1 | SWD flash of uncommitted P10-9 image: GDB `target extended-remote :3333` while vehicle was in `qv_idle_bridge` (once also `init_pio_safety`), then `reset halt` + `load` + `resume`. Second load + inspect-`halt` after CDC miss, OpenOCD left running. No telnet reset-halt before attach; no extra post-flash telnet restart. | OpenOCD: `clearing lockup after double fault`. Core0 Handler HardFault `pc=0xeffffffe` `msp=0x20081f58`; Core1 `pc=0x000000da`. No LED. COM5 ghost / banner peek fail. Load size 226488 @ `0x1000014c` had succeeded. | Kill OpenOCD (was holding HardFault halt). Full VBUS + probe unplug required; Feather-only unplug is not enough. Did not flash again on the E2'd chip. Rescue-DP not tried. | unknown | Operator sequence, not the sitting's `src/` diff. Stacked PC/LR was idle/bootrom, not the patch. Recipe now in `docs/FLASHING.md`; LL Entry 46. |
| 2026-08-20 | R-1 then R-2 | NOLINT-bucket flash: `picotool load -f --ser 02FBDDB8E1CA1281`, then OpenOCD `program`/`halt`/`reset halt` for 3-boot. No extra post-flash reboot before first `bench_sim`. | E2 lockup: Core0 PC=`0xeffffffe` LR=`0xffffffe9` IPSR=3; Core1 bootrom `0x000000da`; COM5 ghost/no banner. Stacked PC/LR was `clock_get_hz` ← `psram_calc_timing` (**not** in the NOLINT diff). | Feather USB unplug **alone** did not clear (probe residual power). Full VBUS cut + probe depower → green LED, CDC, `bench_sim` 2/2 ×3. SWD `reset` without that power cut did not recover. Rescue-DP not tried (replug was enough). | unknown | Do **not** patch the stacked PC when it is outside the session diff. First `bench_sim` 2/2 before extra reboot may have been leftover image. |
