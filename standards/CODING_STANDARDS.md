...
- **Pointer casts (JSF-182)** — the rule bans casting to/from pointers, with **Exception 1** (`void*`→`T*` via `static_cast`, in low-level memory-management routines) and **Exception 2** (literal hardware-address→pointer). The 2026-06-23 L2-P5 walk inventoried every `reinterpret_cast` in `src/` and dispositioned per category: **literal/HW-address→pointer = compliant** (Exception 2, e.g. NVIC `0xE000E200`, `XIP_BASE`, PSRAM bases); **struct→`uint8_t*` for CRC = remediated** (CRC primitives take `const void*`; call sites convert implicitly; the lone internal cast is Exception 1); **buffer→header overlay = remediated** (`reinterpret_cast`→`static_cast`, Exception 1's low-level-memory-management domain); **QP/C `QEvt*`→derived event = centralized** into one `static_assert`-guarded `evt_cast<E>` helper. **Two native-code residuals** the rule bans with no applicable JSF exception — both justified by reasoning *external* to the ruleset (not a JSF exception clause), and therefore logged in `ACCEPTED_STANDARDS_DEVIATIONS.md`: **CAST-1** (MPU linker-symbol→`uintptr_t`; hardware-interface-required, truncation rationale provably inapplicable on a 32-bit target) and **CAST-2** (the `evt_cast` `QEvt*`→derived downcast; well-defined via the C++ standard-layout first-member guarantee, alignment-UB harm cannot occur; QP/C++ would retire it). The compliant categories above (Exception 1/2) are ruleset-internal and need no row. The earlier triage framing that "all of JSF-182 is a standing deviation" is an over-claim, corrected.
