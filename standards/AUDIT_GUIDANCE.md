...
### B.3 FMEA Worksheet Columns (per NASA SWE §8.05 (SW FMEA))
...
**SFMEA master worksheet columns** (NASA SWE §8.05 (SW FMEA)):
...
- **Severity classification** — one of: catastrophic, critical, marginal, negligible (NASA SWE §8.05 (SW FMEA) four-grade scale).
- **Likelihood** — one of: probable, occasional, remote, improbable.
- **Risk level** — 1 (prohibitive — must redesign) through 5 (acceptable). NASA SWE §8.05 (SW FMEA) 5-level scale.
...
The A.1 table columns ("Failure Mode," "Files / Sections," "What to Verify," "Positive-Control Signal," "Status") collapse the full NASA SWE §8.05 (SW FMEA) worksheet into a tight 5-column form suitable for a single-page review:
...
**Software review & inspection methodology (review/audit guidance — governs *how we review*, NOT a coding/style standard):**
- **IEEE Std 1028-2008, *Standard for Software Reviews and Audits*** — https://ieeexplore.ieee.org/document/4601584/ (the umbrella review-process standard; specifies **five** review types — management review, technical review, **inspection**, walk-through, audit — with roles and procedures for each. It defines *how* a review is conducted, not *when* to run one and not what a coding rule is.) The L2-P5 manual walk is an **inspection** in this taxonomy (see Appendix B.4 "Phase 9 — Post-audit guided code review").
- **Fagan, M. (1976), "Design and Code Inspections to Reduce Errors in Program Development," *IBM Systems Journal* 15(3):182–211** — https://dl.acm.org/doi/10.1147/sj.153.0182 (the origin of formal software inspection; the roles/phases model IEEE 1028's inspection type formalizes).

**Distinction — deliberate.** These are *review/audit-process* standards (how we conduct a review) and are kept separate from the *coding/style* standards below (JSF / P10 / JPL, which govern how the code is written). A finding of "you didn't inspect X" is an IEEE-1028 matter; a finding of "X violates a rule" is a coding-standards matter. **Provisional scope (not settled):** IEEE 1028 is recorded here as a useful but *broad* starting reference for review process — **not** a fully-evaluated or *sole* review standard. It has had limited project vetting so far; complementary review standards may well sit alongside it, to be considered when this area is next reworked. **Future-rework hook:** the review-*depth* per trigger in the "When to Do What" decision table above is currently implicit — a later rework can map IEEE 1028's five review levels onto our 7 tiers so "which depth of review for which change" is explicit.
...
- NASA Software Engineering Handbook — https://swehb.nasa.gov/ (handbook root; publicly accessible NASA-wide guidance)
  - §8.05 — Software Failure Modes and Effects Analysis (source for B.3 above)
  - §8.11 — Auto-Generated Code (source for CG-1 deviation rationale)

**Safety-critical FMEA primary sources** (MIL-STD-1629A and SAE ARP4761 — both predate NASA SWE §8.05 (SW FMEA) which now serves as the project-applicable digest):
- MIL-STD-1629A (U.S. DoD, 1980) — "Procedures for Performing a Failure Mode, Effects and Criticality Analysis." Hosted via NASA at https://extapps.ksc.nasa.gov/Reliability/Documents/milstd1629_FMEA.pdf (PDF text-layer extraction has been unreliable; consult NASA SWE §8.05 (SW FMEA) first).
- SAE ARP4761 — Aerospace Recommended Practice for safety assessment process on civil airborne systems. SAE-paywalled; for our project's needs, NASA SWE §8.05 (SW FMEA) (which references both upstream) is the project-applicable digest.
...
- **NASA SWE §8.05 (SW FMEA) severity scale** is the project's framing for C.3 category 3's within-tier ordering. NASA's catastrophic/critical/marginal/negligible scale combines with mission-phase exposure (flight-critical vs ground-only) to produce the actual risk rank.
