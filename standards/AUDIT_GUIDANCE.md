...
**Distinction — deliberate.** These are *review/audit-process* standards (how we conduct a review) and are kept separate from the *coding/style* standards below (JSF / P10 / JPL, which govern how the code is written). A finding of "you didn't inspect X" is an IEEE-1028 matter; a finding of "X violates a rule" is a coding-standards matter. **Future-rework hook:** the review-*depth* per trigger in the "When to Do What" decision table above is currently implicit — a later rework can map IEEE 1028's five review levels onto our 7 tiers so "which depth of review for which change" is explicit.
