                                                            ...
                                                            uint64_t sample_us)
{
...

    {
        ...
        // PD12 fix: Use memory_order_release to synchronize with main loop reads
                                                             ...
                                                             bool fsync_set)
{
...

    {
        ...
        // PD12 fix: Use memory_order_release to synchronize with main loop reads
        _imu._new_accel_data[instance].store(true, std::memory_order_release);
    }
...
{    
    ...
    // PD12 fix: Use memory_order_acquire to synchronize with callback writes
    if (_imu._new_gyro_data[instance].load(std::memory_order_acquire)) {
...
#endif
        // PD12 fix: Use memory_order_relaxed for clear (no ordering needed)
...
{    
    ...
    // PD12 fix: Use memory_order_acquire to synchronize with callback writes
    if (_imu._new_accel_data[instance].load(std::memory_order_acquire)) {
        _publish_accel(instance, _imu._accel_filtered[instance]);
        // PD12 fix: Use memory_order_relaxed for clear (no ordering needed)
        _imu._new_accel_data[instance].store(false, std::memory_order_relaxed);
    }
